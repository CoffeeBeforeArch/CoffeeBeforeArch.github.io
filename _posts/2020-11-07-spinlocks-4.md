...
You can see that the performance of the two optimizations are roughly the same. The slight differences are to be expected because thread scheduling is non-deterministic, and is heavily influenced by the current load of the CPUs. Let's move onto the primary metric we were targeting with this optimization (power consumption).

### Power

To reason about the power-consumption of our applications, we'll be looking at the total number of instructions executed to compare the amount of work being done, then directly querying power performance counters.
