...
Each set has only a limited number of ways (usually something on the order of 4, or 8). While our cache sizes may be relatively large (at least compared to the size of our matrices), we may not be able to make effective use of them if every cache line we access is fighting for the same 4 or 8 ways in a single set. Let's try slightly increasing the size of our matrix, and re-collect the performance numbers. We'll use 2^8 + 16, 2^9 + 16, and 2^10 + 16. Here are the performance numbers:
