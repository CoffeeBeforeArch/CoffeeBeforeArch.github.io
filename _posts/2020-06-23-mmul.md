...
We can parallelize our blocked implementation similar to how we parallelized our original serial implementation. Here is my code for that:
...
All the inner-loop code stayed the same. We just had to modify our outer loop to start at the `start_row`, and end at the `end_row` instead of going over the entire matrix.

![blocked parallel](/assets/mmul/blocked_parallel/.png)

Let's see if we improved our performance at all.

```
-----------------------------------------------------------------------------------
Benchmark                                         Time             CPU   Iterations
-----------------------------------------------------------------------------------
parallel_blocked_gemm_bench/8/real_time        4.77 ms        0.696 ms          598
parallel_blocked_gemm_bench/9/real_time        20.6 ms         4.42 ms          126
parallel_blocked_gemm_bench/10/real_time        251 ms         41.9 ms           12
```

Our performance... stayed pretty much the same? Despite the fact we through more threads at the problem, we didn't seem to make any headway into improving performance. In fact, our performance looks like it got worse by adding threads. To make matters more confusing, let's look at another run from our benchmark.

```
-----------------------------------------------------------------------------------
Benchmark                                         Time             CPU   Iterations
-----------------------------------------------------------------------------------
parallel_blocked_gemm_bench/8/real_time        5.22 ms        0.720 ms          568
parallel_blocked_gemm_bench/9/real_time        15.1 ms         4.36 ms          107
parallel_blocked_gemm_bench/10/real_time        117 ms         42.1 ms           12
```

Now our performance is better (in all but the smallest matrix size). There are a number of things at play here. For one, we have to pay the cost of creating threads and joining threads, which will only be amortized at larger matrix sizes. However, probably more important in this case, is cache contention. We know have multiple threads accessing different parts of the matrix, and competing for the same bus bandwidth. This can lead to inconsistent performance results. Our threads could also be bouncing between different cores with different local L1 and L2 caches where they will have to re-fetch all the data again.
...
We now have a single-threaded version that is faster than our baseline implementation for all cases!

## Parallel Blocked Column

Let's parallelize our blocked column algorithm. In this case, we have to do things slightly differently. Because our algorithm is based around blocks of columns, we must give a dedicated block of columns to each thread. This makes things slightly difficult, because our simple code relies on that being a multiple of 16. However, we can get around this fairly easily by having each thread work on only 16 columns at a time, and come back for more when they are done. Here is how that was implemented.
...
Each iteration of the outer loop, the starting column number is given by the call to `pos.fetch_add(16)`. This increments the current global column number by 16, and return the previous value. While each thread pays the price for finding what chunk of columns they are working on next, this is is relatively small, as it happens only once in the outer-most loop. Furthermore, it allows us to keep the code fairly simple.
