...
Let's consider a very simple function called `add`, that takes two integers by references, sets each to a value, and returns their sum. Let's first set our input references `a` and `b` to the same value, and return their sum.
...
I like to use [Compiler Explorer](https://godbolt.org) for my to look at source and assembly side-by-side. Here is my results with `O3` optimizations enabled.

![assert enabled](/assets/aliasing/aliasing_hidden.png)

Nothing terribly surprising in the output, and the compiler seems to perform the optimizations we'd expect. It sets the value of memory locations `a` and `b` to 5, and precomputes the resulting sum for the return. Simple enough. Now let's see what happens if we change `a` and `b` to be different values.
...
Two major things seem to have happened. For one, we lost our precomputed resulting sum. Secondly, we seem to be loading `a` into the `eax` register after setting it. Why did these things happen? Aliasing! What's going on here is that the compiler does not have enough context to determine if `a` and `b` refer to the same location in memory. As a result, the compiler must be conservative, and assume that they could possibly point to the same memory location. Let's consider a simple example of why the compiler makes this decision.
...
The final value of result should clearly be 9 (just the simple addition of 5 + 4). Seems simple enough to precompute the value. However, consider this slightly different call.
...
But what if we disabled those rules? Let's see what happens if we add the compiler flag `-fno-strict-aliasing`, allowing the compiler to thing that an integer and floating point number can alias each other.
...
Ok, so now we have some insight into aliasing, and how it can lead to less optimal code. Let's take a step back and discuss how we can solve this problem with our original `add` funcion. In the programming langue C, we can solve this problem by adding the `restrict` qualifier to our references. This is just a hint to the compiler that pointer or references will not alias each other. Fortran solves this problem by simply not allowing paramters to be aliases. But what about C++?

C++ does not have a `restrict` keyword. However, most every major compiler supports an intrinsic to mark that references or pointers do not alias each other. In the case of gcc, we can use `__restrict`. Here's our modified `add` function.
...
Our optimizations came back! The compiler just needed a little extra information to provide us with more optimal code!
...
We seeme to be clearly bottlenecked on this gather, but that is a problem for another time. What we will focus on is the fact that the compiler seems to have used a lot of vector instructions to implement matrix multiplication.
...
Our vectorization went away! But what happened? Our matrix multiplication and benchmark code is identical to the first example, but is just in different translation units. Let's think back to earlier when we discussed alias analysis and context.

When we compile our matrix multiplication code with the rest of the benchmark in the same translation unit, the compiler can determine that the pointers passed in to the `base_mmul` are not aliases of each other, and optimize the function accordingly. However, when `base_mmul` is compiled separately from the call-site, this analysis and extra optimization is not done by default during linking.
...
The assembly from our original code that does not use temporaries shows a very different story. Without posting the entire assembly, the highlights is that we first see the zeroing out of elements in the `c` matrix.

```asm
/*00e0*/                   STG.E.SYS [R2], RZ ;                               /* 0x000000ff02007386 */
```

Afterwards, there are multiple stores to the same address stored in register `R2` (the `c` matrix), writing back the partial results.

```asm
/*0290*/                   STG.E.SYS [R2], R5 ;                               /* 0x0000000502007386 */
```

Aliasing again! The compiler can not determine if `a`, `b`, and `c` alias each other. This means that the compiler has to assume that when we write to `c`, we could also be writing to `a` or `b` as well (if they are aliases). The compiler can not optimize away the stores into writes to a register until the final update of the `c` matrix.

Why didn't this occur in the version with the temporary variable? When we declare that variable `tmp` it is located on the stack, in private per-thread local storage. The compiler can fairly easily decide to just place this inside of a register (`R27` in this case).

Fortunately, we can give the compiler some extra help. Just like we did with our simple `add` function, we use the `__restrict` intrinsic to inform the compiler that our pointers are not aliases of each other. The full code can be found [here](https://github.com/CoffeeBeforeArch/spring_2020_tutorial/blob/master/aliasing/matrixMul.cu)
...
Measuring the execution time, we find huge improvement over the orignal code, and much closer to the version using the temporary (~1.xms). Looking at the assembly, we find only 2 stores! The first corresponds to the zeroing out of the element of the `c` matrix.
