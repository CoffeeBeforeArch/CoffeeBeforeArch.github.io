...
The first thing the `pthread_spin_lock` routine does is try and grab the lock using an atomic decrement (`LOCK decl`). Here, a thread gets the lock when it decrements the  `pthread_spinlock_t` value in `(%rdi)` from `1` to `0` (this will set the zero flag). If the zero flag is set, the thread exits the routine, returning `0`.
...
Fundamentally, it is a spinlock doing the same passive backoff optimization with some small hand-tuned differences (e.g., instruction scheduling and using an atomic decrement instead of an atomic exchange).
...
Instead of duplicating code for init and unlock, the exact same routine is used.
...
If we compare this to our own passive backoff spinlock performance, we see that the performance is fairly close:
...
However, there are a few things to note. For one, the pthread spinlock is significantly faster in single-thread case. This shows good efficient design (compared to ours) because the spinlock should be incredibly fast when there is no contention.
...
We've come to the end of the last post (for now) on spinlocks. What we've seen is that it doesn't take much to write a functionally correct implementation, but it takes quite a lot of thought to make one with good performance. We've also seen that a library implementation isn't necessarily the end-all be-all for performance. With a little work, you can write your own implementation that works best for your scenario.
