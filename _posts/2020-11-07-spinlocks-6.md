...
Similar to the previous assembly, our threads first to an atomic fetch and add to get a ticket in line. Then, we read the now-serving number into the register `esi`. We then fall into a 2 instruction loop at label `1f:` where we compare the values of two registers (our number in line and the now-serving number) until they are no longer equal.
...
If the `lock` is already taken, the thread gets its place in line, reads the now serving position into a register again, and we compare our place in line to now-serving position (both in registers) forever, finding that they are never equal. They will never be equal because the thread that releases the lock increments the now-serving position in memory. This does not affect values stored in registers (cache coherence does not apply to registers!).

Our threads waiting in the loop at `1f:` will never see the update in memory.
