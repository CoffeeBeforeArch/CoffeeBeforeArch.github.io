...
The first thing our threads do is an atomic fetch and add (`xadd`) to get their place in line. Then the threads wait inside of the loop at label `28:`, where the threads read the now serving number.

If a thread finds its now serving number is equal to its ticket number, it breaks out of the loop, increments the shared value (`incq`), then increments the now-serving number and stores it to memory, and starting the loop over again.

### Removing the Volatile Qualifier

I mentioned previously that we needed the `volatile` qualifier to prevent the compiler from caching the now-serving number in a register. Let's look at the assembly generated with the `volatile` is removed:
...
Similar to the previous assembly, our threads first do an atomic fetch and add to a place in line. Next, they read the now-serving number into the register `esi`. From there, the threads fall into a 2 instruction loop at label `1f:` where the values of two registers (our number in line and the now-serving number) are compared until they are no longer equal.

However, if any thread tries to grab the lock while it is still taken, they will never break out of this loop! Let's think about why. If the lock is free, and one thread tries to grab the lock, the thread grabs a place in line, reads the now serving number into a register, finds that it is equal to its place in line, and exits the `lock` method.

If the `lock` is already taken, the thread gets its place in line, reads the now serving position into a register again, and we compare our place in line to now-serving position (both in registers) forever, finding that they are never equal. This happens because the thread that releases the lock increments the now-serving position in memory, which does not affect values stored in registers (cache coherence does not apply to registers!).
...
Our performance isn't great compared to previous implementations (e.g., exponential backoff) but it's not terrible. In fact, it's fairly close to our passive backoff implementation. As a reminder, here are those performance numbers:
...
In our ticket spinlock, we're doing almost the opposite. We're now ensuring that the lock will be passed between requesting threads in first-in first-out order. That means that our cache lines will be bouncing between the cores of requesting threads, which adds some extra overhead.

## Thoughts on Compiler Memory Re-Ordering

One of the potential pitfalls we did not discuss was memory re-ordering by the compiler. Specifically, `volatile` does not guarantee the "happens before" relationship, meaning that non-volatile store can be reordered and placed after a `volatile` store, even if it occurs earlier in the program.

To prevent this, we can insert a compiler barrier in our `unlock` method:
...
If all of these `volatile` and memory ordering details are getting too confusing, we can always replace our now-serving number with an atomic. By default, that will ensure that no-reordering will occur (with the default `std::memory_order_seq_cst`). Here is the resulting assembly:
...
We see roughly the same code as our example with the compiler memory barrier with the exception that we use an atomic increment (`lock incw`) to release the lock. If we look at the performance, we see that our performance using an atomic made things even worse, and for all cases:
...
We also saw some gory details about how parallel programming and synchronization can be difficult! There are many corner cases in parallel programming, and memory ordering bugs can be incredibly subtle. While the heavy hammer of an atomic helped us avoid some of the gory details of inline assembly to avoid compiler memory re-ordering, it came at the cost of our performance. 

Knowing what tools you have in your toolbox, and how your language, compiler, and processor function are critical to writing fast, but more importantly, correct code.
